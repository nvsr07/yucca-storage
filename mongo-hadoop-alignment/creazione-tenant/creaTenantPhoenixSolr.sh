...
#phoenix-sqlline-thin sdnet-edge1.sdp.csi.it <<EOF
...
###
urlSolr="http://sdnet-solr.sdp.csi.it:8983/solr/admin/collections?action="
...
while [ $i -lt 4 ] ; do
  ...
  curl ${urlSolr}"LIST&wt=json" | grep -i '"'${myColl}'"' >/dev/null
  if [ $? -eq 0 ]; then
    ...
    rispostaCurl=`curl -v "${urlSolr}CREATE&name=${myColl}${optSolrCurr}&collection.configName=${myColl}"`
    if [ $? -ne 0 ] ; then
    ...
    rispostaCurl=`curl -v "${urlSolr}ADDREPLICA&collection=${myColl}&shard=shard1&node=sdnet-slave"$((i+6))".sdp.csi.it:8983_solr"`
    if [ $? -ne 0 ]; then
    ...
    rispostaCurl=`curl -v "${urlSolr}ADDREPLICA&collection=${myColl}&shard=shard2&node=sdnet-slave"$((i+7))".sdp.csi.it:8983_solr"`
    if [ $? -ne 0 ]; then
