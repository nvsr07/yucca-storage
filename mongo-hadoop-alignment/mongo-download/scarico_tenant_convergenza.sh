...
# crea lista tenant
mongo $MONGO_HOST:$MONGO_PORT/DB_SUPPORT -u $MONGO_USER -p $MONGO_PWD --authenticationDatabase admin --quiet elenco_tenant_organizzazione.js > $nomeDir/lista_tenant_org.$myPid.txt
if [ $? -ne 0 ]
then
...
# crea lista dataset
mongo $MONGO_HOST:$MONGO_PORT/DB_SUPPORT -u $MONGO_USER -p $MONGO_PWD --authenticationDatabase admin --quiet --eval "var param1='$tenantCode'" elenco_dataset_convergenza.js > $nomeDir/lista_dataset.$myPid.txt
if [ $? -ne 0 ]
then
...
#
 echo campi=$campi
 echo mongoexport -h $MONGO_HOST --port $MONGO_PORT -u $MONGO_USER -p $MONGO_PWD --authenticationDatabase admin -d $database -c $collection -q "{ \$and: [ {idDataset: $idDataset}, {datasetVersion: $datasetVersion}, { _id: { \$gte: ObjectId(\"$precTS\") } }, { _id: { \$lt: ObjectId(\"$newTS\") } }, { \$or: [ {origin:{\$exists: false}}, {origin:{\$ne:\"datalake\"}} ] } ] }" --type=csv -o $nomeDir/export_dataset.$myPid.txt -f $campi
 mongoexport -h $MONGO_HOST --port $MONGO_PORT -u $MONGO_USER -p $MONGO_PWD --authenticationDatabase admin -d $database -c $collection -q "{ \$and: [ {idDataset: $idDataset}, {datasetVersion: $datasetVersion}, { _id: { \$gte: ObjectId(\"$precTS\") } }, { _id: { \$lt: ObjectId(\"$newTS\") } }, { \$or: [ {origin:{\$exists: false}}, {origin:{\$ne:\"datalake\"}} ] } ] }" --type=csv -o $nomeDir/export_dataset.$myPid.txt -f $campi
 if [ $? -ne 0 ]
 then
 ...
 then
   ...
   totale_scaricate=$(($totale_scaricate + $righe_file -1))
   ...
   nome_file_dati="$nomeDir/${newTS}.${tenantCode}.${tenantOrg}.${dominio}.${sottoDominio}.${collection}.${streamCode}.${vESlug}.${datasetCode}.${datasetVersion}.${visibility}.csv"
