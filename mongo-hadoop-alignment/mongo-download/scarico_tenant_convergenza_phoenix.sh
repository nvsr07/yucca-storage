...
# crea lista tenant
mongo $MONGO_HOST:$MONGO_PORT/DB_SUPPORT -u $MONGO_USER -p $MONGO_PWD --authenticationDatabase admin --quiet elenco_tenant_organizzazione_phoenix.js > $nomeDir/lista_tenant_org.$myPid.txt
if [ $? -ne 0 ]
then
...
dataPhoenixTableName=`echo $tenantInfo | awk -F\; '{print $3}'`
measuresPhoenixTableName=`echo $tenantInfo | awk -F\; '{print $4}'`
socialPhoenixTableName=`echo $tenantInfo | awk -F\; '{print $5}'`
mediaPhoenixTableName=`echo $tenantInfo | awk -F\; '{print $6}'`
...
# crea lista dataset
mongo $MONGO_HOST:$MONGO_PORT/DB_SUPPORT -u $MONGO_USER -p $MONGO_PWD --authenticationDatabase admin --quiet --eval "var param1='$tenantCode'" elenco_dataset_convergenza_phoenix.js > $nomeDir/lista_dataset.$myPid.txt
if [ $? -ne 0 ]
then
...
do
 ...
 campiPhoenix=`echo $riga | awk -F\; '{print $14}'`
...
# add time field, just for measures, social and media collection
 ...
 then
  ...
  phoenixTable=$measuresPhoenixTableName
 ...
 then
  ...
  timestamp_field=""
  phoenixTable=$dataPhoenixTableName
 elif  [ $collection == "social" ]
 then
  echo does not enriches the file
  script_metadati="metadati_empty.js"
  timestamp_field="time_dt,"
  phoenixTable=$socialPhoenixTableName
 elif  [ $collection == "media" ]
 then
  echo does not enriches the file
  script_metadati="metadati_empty.js"
  timestamp_field="time_dt,"
  phoenixTable=$mediaPhoenixTableName
 else
  echo does not enriches the file
  script_metadati="metadati_empty.js"
  timestamp_field=""
...
#
 echo campi=$campi
 echo "SELECT $campi FROM $phoenixTable($campiPhoenix) WHERE IDDATASET_L = $idDataset AND DATASETVERSION_L = $datasetVersion AND ID > '$precTS' AND ID < 'newTS';" 
 
 echo "!brief" > export_query.$myPid.sql
 echo "!set showwarnings false" >> export_query.$myPid.sql
 echo "!set showelapsedtime false" >> export_query.$myPid.sql
 echo "!set headerinterval 0" >> export_query.$myPid.sql
 echo "!set silent true" >> export_query.$myPid.sql
 echo "!set incremental true" >> export_query.$myPid.sql
 echo "!outputformat csv" >> export_query.$myPid.sql
 echo "SELECT $campi FROM $phoenixTable($campiPhoenix) WHERE IDDATASET_L = $idDataset AND DATASETVERSION_L = $datasetVersion AND ID > '$precTS' AND ID < 'newTS';" >> export_query.$myPid.sql
 
 phoenix-sqlline sdnet-master1.sdp.csi.it,sdnet-master2.sdp.csi.it,sdnet-master3.sdp.csi.it,sdnet-master4.sdp.csi.it,sdnet-master5.sdp.csi.it export_query.$myPid.sql > $nomeDir/export_dataset.$myPid.txt 
